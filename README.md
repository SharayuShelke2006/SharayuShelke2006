## Hi there I am Sharayu Shelke👋


🎓 I'm a **B.Tech student in Artificial Intelligence and Data Science (AIDS)** with a strong interest in building intelligent and scalable web applications.

💻 I specialize in full-stack **web development** using the **MERN Stack (MongoDB, Express.js, React, Node.js)** and also enjoy solving coding problems using **Python** and practicing **Data Structures and Algorithms**.

---

### 🚀 Tech Stack

* **Frontend:** HTML, CSS, JavaScript
* **Backend:** Node.js, Express.js
* **Database:** MongoDB, MySQL
* **Programming Languages:** Python, JavaScript, C++,C,Java
* **Tools & Platforms:** Git, GitHub, VS Code, Postman,CodeChef,IBMSkills,leetcode,Hackerrank
* **Currently Learning:**  Advanced DSA, AI/ML,Data science Advance

---

### 🌱 Currently Working On

* Full-stack web apps using MERN
* Strengthening DSA and problem-solving skills
* Exploring machine learning and AI tools

---

### 📫 Connect with Me

* 🔗 [LinkedIn][(https://www.linkedin.com/in/sharayu-shelke-bba44a37b/)] 
* 
* ✨ Always open to learning, collaboration, and exciting new challenges!

---



![](https://leetcard.jacoblin.cool/justsheruu?theme=unicorn)

